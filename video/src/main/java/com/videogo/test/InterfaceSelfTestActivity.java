...
public class InterfaceSelfTestActivity extends Activity implements View.OnClickListener
    ...
    public void onClick(View view) {
        int i = view.getId();
        if (i == R.id.interface_self_test_button) {
            testV32Interface();

        } else if (i == R.id.interface_v33_self_test_button) {
            testV33Interface();

        } else if (i == R.id.id_interface_self_test_openCloudPage) {
        ...
        } else if (i == R.id.id_interface_self_test_forgetPassword) {
            EZOpenSDK.getInstance().openChangePasswordPage();

        } else if (i == R.id.id_show_stream_limit_dialog2) {
...
//                invoke_capturePicture(deviceSerial);
//                invoke_getUserInfo();
//                invoke_getUnreadMessageCount(deviceSerial);
                invoke_getAlarmListBySerial(deviceSerial);
//                invoke_other(deviceSerial);
            }});
        thr.start();
    }
    ...
    private void testV33Interface() {
        ...
        Thread thr = new Thread(new Runnable() {
            @Override
            public void run() {
//                invoke_getDeviceList();
//                invoke_setDeviceDefence();
//                test_decryptData();
    ...
    private void invoke_getAlarmListBySerial(String deviceSerial) {
        ...
        try {
            ...
            result = EZOpenSDK.getInstance().getAlarmListBySerial(deviceSerial, 0, 5, begin, end);
        ...
        try {
            ...
            result = EZOpenSDK.getInstance().getAlarmListBySerial(null, 0, 5, begin, end);
            LogUtil.i(TAG, "invoke_getAlarmListBySerial: " + result);
        } catch (BaseException e) {
            e.printStackTrace();
        }
        ...
        try {
            ...
            result = EZOpenSDK.getInstance().getAlarmListBySerial(null, 0, 5, null, end);
            LogUtil.i(TAG, "invoke_getAlarmListBySerial: " + result);
        } catch (BaseException e) {
//            e.printStackTrace();
        }
        ...
        try {
            ...
            result = EZOpenSDK.getInstance().getAlarmListBySerial(null, 0, 5, begin, null);
            LogUtil.i(TAG, "invoke_getAlarmListBySerial: " + result);
            Assert(true);
        } catch (BaseException e) {
            e.printStackTrace();
        ...
        try {
            ...
            result = EZOpenSDK.getInstance().getAlarmListBySerial(null, 0, 5, null, null);
            LogUtil.i(TAG, "invoke_getAlarmListBySerial: " + result);
...
//            e.printStackTrace();
        ...
        try {
            ...
            result = EZOpenSDK.getInstance().getAlarmListBySerial("12345678a", 0, 5, begin, end);
            LogUtil.i(TAG, "invoke_getAlarmListBySerial: " + result);
            Assert(false, "");
        } catch (BaseException e) {
//            e.printStackTrace();
...
//                LogUtil.i(TAG, "test_decryptData:compressed to file");
                ...
                case MSG_PLAY_NEXT:
                    if(mPlayIndex < mLeaveMsgList.size() ) {
                        Thread thr = new Thread(new Runnable() {
                            @Override
                            public void run() {
                                boolean result = EZOpenSDK.getInstance().getLeaveMessageData(mHandler, mLeaveMsgList.get(mPlayIndex), InterfaceSelfTestActivity.this);
    ...
    private void showLimitDialog(Context context, int type, int limit_minutes) {
        ...
        passwordErrorLayout.setId(R.id.new_password_error_layout);
