...
public class EZRemotePlayBackActivity extends Activity implements OnClickListener, SurfaceHolder.Callback, 
    Handler.Callback, TimeScrollBarScrollListener, SecureValidate.SecureValidateListener, OpenYSService.OpenYSServiceListener {
...
//                handleSearchEZDeviceFileSuccess();
    ...
    private void handleSearchEZCloudFileSuccess(Calendar seletedTime) {        
        if (mEZMediaPlayer == null) {
            ...
            mEZMediaPlayer = mEZOpenSDK.createPlayer(this, cameraId);
		...
		if ((mEZDeviceFileList != null && mEZDeviceFileList.size() > 0)
			...
			if (mAlarmStartTime != null) {
	            ...
	            if (ezDeviceFile != null) {
	                mEZMediaPlayer.startPlayback(ezDeviceFile);
	            }
			}
		} else {
			handleSearchNoFile();
    ...
    public void onClick(View view) {
        int i = view.getId();
        if (i == R.id.remoteplayback_loading_play_btn) {
            startRemotePlayBack(getTimeBarSeekTime());

        } else if (i == R.id.remoteplayback_play_btn || i == R.id.remoteplayback_full_play_btn) {
        ...
        } else if (i == R.id.remoteplayback_replay_btn) {
            if (mStatus != STATUS_STOP) {
                stopRemotePlayBack();
            }
            startRemotePlayBack(null);

        } else if (i == R.id.remoteplayback_sound_btn || i == R.id.remoteplayback_full_sound_btn) {
            onSoundBtnClick();

        } else if (i == R.id.remoteplayback_previously_btn || i == R.id.remoteplayback_full_previously_btn || i == R.id.remoteplayback_small_previously_btn) {
            onCapturePicBtnClick();

        } else if (i == R.id.remoteplayback_capture_rl) {
            onCaptureRlClick();

        } else if (i == R.id.remoteplayback_video_btn || i == R.id.remoteplayback_full_video_btn || i == R.id.remoteplayback_small_video_btn || i == R.id.remoteplayback_video_start_btn || i == R.id.remoteplayback_full_video_start_btn || i == R.id.remoteplayback_small_video_start_btn) {
            onRecordBtnClick();

        } else if (i == R.id.remoteplayback_full_down_btn) {
    ...
    private void onRecordBtnClick() {
        ...
        if(mEZMediaPlayer != null) {
            ...
            boolean result = mEZMediaPlayer.startLocalRecord(null);
            if(result) {
            	bIsRecording = !bIsRecording;
            }
        }
...
//        mRemotePlayBackCaptureRl.setVisibility(View.VISIBLE);
                ...
                public void run() {
                    ...
                    if(bmp != null) {
                        try {
                            ...
                            String path = EZUtils.generateCaptureFilePath(mLocalInfo.getFilePath(), mCameraInfo.getCameraId(), serial);
                            ...
                            runOnUiThread(new Runnable(){
                                @Override
                                public void run() {
                                    Toast.makeText(EZRemotePlayBackActivity.this, getResources().getString(R.string.already_saved_to_volume), Toast.LENGTH_SHORT).show();
                                }});
    ...
    private void searchEZCloudFileList(final Calendar seletedTime) {
        new Thread() {
            ...
            public void run() {
                try {
                    mEZCloudFileList = mEZOpenSDK.searchRecordFileFromCloud(mCameraInfo.getCameraId(), mStartTime, mEndTime);
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    ...
                    mEZDeviceFileList = mEZOpenSDK.searchRecordFileFromDevice(mCameraInfo.getCameraId(), startTime, endTime);
                    if(mEZDeviceFileList != null && mEZDeviceFileList.size() > 0) {
                    ...
                    mEZCloudFileList = mEZOpenSDK.searchRecordFileFromCloud(mCameraInfo.getCameraId(), startTime, endTime);
...
//        mRemotePlayBackCaptureRl.setVisibility(View.VISIBLE);   
        ...
        if(OSDTime != null) {
            ...
            if(mAlarmStartTime != null) {
                ...
                LogUtil.i(TAG, "updateRemotePlayBackUI mPlayTime:" + mPlayTime + "mAlarmStartTime:"+mAlarmStartTime.getTime()
                		+" mAlarmStartTime:" + mAlarmStartTime.getTimeInMillis());
    ...
    private void handlePlayProgress(Calendar osdTime) {
        ...
        long begin = mAlarmStartTime.getTimeInMillis();
