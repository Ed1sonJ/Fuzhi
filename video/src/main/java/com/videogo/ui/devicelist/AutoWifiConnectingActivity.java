...
//            mDeviceInfoEx = DeviceManager.getInstance().getDeviceInfoExById(serialNo);
    ...
    private void init() {
        ...
        isSupportNetWork = getIntent().getBooleanExtra(ChooseDeviceModeActivity.SUPPORT_NET_WORK, true);
        isSupportWifi = getIntent().getBooleanExtra(ChooseDeviceModeActivity.SUPPORT_WIFI, true);
...
//                            LocalInfo.getInstance().setWifiPassword(mac, wifiPassword);
                ...
                Thread thr = new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (result == 0 && mEZProbeDeviceInfo != null) {
                            ...
                            // TODO
                        } else if (result == EZOpenSDKError.ERROR_WEB_DIVICE_ONLINE_NOT_ADD) {
    ...
    public void onClick(View v) {
        ...
        } else if (i == R.id.btnLineConnet) {
            lineConnectClick();

        } else if (i == R.id.btnLineConnetOk) {
            lineConnectOkClick();

        } else if (i == R.id.btnFinish) {
            finishOnClick();

        } else if (i == R.id.tvMore) {
            moreOnClick();
    ...
    private void addCameraFailed(int errorStep, int errorCode) {
        ...
        switch (errorStep) {
            ...
            case ERROR_ADD_CAMERA:
                ...
                if (errorCode == EZOpenSDKError.ERROR_WEB_DEVICE_EXCEPTION) {
                    // 设备异常
                    tvStatus.setText(getString(R.string.auto_wifi_add_device_failed) + "("
                            + getString(R.string.device_error) + ")");
                } else if (errorCode == EZOpenSDKError.ERROR_WEB_DEVICE_ADD_OWN_AGAIN) {
                    // 设备已被自己添加
                    ...
                    btnRetry.setVisibility(View.GONE);
                    btnFinish.setVisibility(View.VISIBLE);
                } else if (errorCode == EZOpenSDKError.ERROR_WEB_SET_EMAIL_REPEAT_ERROR
                        || errorCode == EZOpenSDKError.ERROR_WEB_CAMERA_ADDED_MORE) {
                    // TODO
                    // 设备已经添加
                    tvStatus.setText(getString(R.string.auto_wifi_add_device_failed2) + "("
                    ...
                    btnRetry.setVisibility(View.GONE);
                    btnFinish.setVisibility(View.VISIBLE);
                } else if (errorCode == EZOpenSDKError.ERROR_WEB_NET_EXCEPTION) {
                    // 网络异常
                    tvStatus.setText(getString(R.string.auto_wifi_add_device_failed2) + "("
                            + getString(R.string.network_exception) + ")");
                } else if (errorCode == EZOpenSDKError.ERROR_WEB_DIVICE_NOT_ONLINE) {
                    // 设备不在线
                    tvStatus.setText(R.string.add_device_failed_not_online);
                } else if (errorCode == EZOpenSDKError.ERROR_WEB_VERIFY_CODE_ERROR) {
                    // 验证码错误
                    tvStatus.setText(getString(R.string.auto_wifi_add_device_failed2) + "("
                            + getString(R.string.verify_code_error) + ")");
                } else if (errorCode == EZOpenSDKError.ERROR_WEB_DEVICE_NOTEXIT) {
                    // 设备不存在
                    tvStatus.setText(R.string.auto_wifi_device_not_exist);
                } else if (errorCode == EZOpenSDKError.ERROR_WEB_QUERY_CAMERA_REPEAT_ERROR) {
                    // 设备已被其他人添加
                    tvStatus.setText(R.string.auto_wifi_device_added_by_others);
                    btnRetry.setVisibility(View.GONE);
                    btnFinish.setVisibility(View.VISIBLE);
                } else if (errorCode == EZOpenSDKError.ERROR_WEB_DIVICE_OFFLINE_NOT_ADD) {
                    // 设备不在线 未添加
        ...
        new Thread() {
            public void run() {
                ...
                while (count > 0) {
                    ...
                    try {
                        mEZOpenSDK.addDevice(serialNo, mVerifyCode);
                        
//                        mDeviceInfoEx = CameraMgtCtrl.addCamera(mSearchDevice.getSubSerial(), mVerifyCode);

                        if (!TextUtils.isEmpty(mVerifyCode)) {
                            //保存密码
                            mEZOpenSDK.setValidateCode(mVerifyCode, serialNo);
                        }
