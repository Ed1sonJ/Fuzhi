...
import com.videogo.openapi.EZConstants.EZOpenSDKError;
...
//        mTriggerHelper = new UnbindDeviceTriggerHelper(this, R.id.unbind_button);
    ...
    public void onClick(View view) {
        int i = view.getId();
        if (i == R.id.searchBtn) {// 只有手动搜索点击的时候才会，清空验证码，二维码过来的不会清空
        ...
        } else if (i == R.id.myRetry) {
            searchCameraBySN();

        } else if (i == R.id.activateHint) {
...
//                mTriggerHelper.onNewIntent(intent);
    ...
    private void handleAddCameraFail(int errCode) {
        ...
        mWaitDlg.hide();
        switch (errCode) {
            case EZOpenSDKError.VIDEOGONETSDK_NETWORD_EXCEPTION:
                showToast(R.string.add_camera_fail_network_exception);
                break;
            case EZOpenSDKError.VIDEOGONETSDK_SESSION_ERROR:
                ActivityUtils.handleSessionException(SeriesNumSearchActivity.this);
                break;
            case EZOpenSDKError.ERROR_WEB_HARDWARE_SIGNATURE_ERROR:
                ActivityUtils.handleHardwareError(SeriesNumSearchActivity.this, null);
                break;
            case EZOpenSDKError.VIDEOGONETSDK_SERVER_EXCEPTION:
                showToast(R.string.add_camera_fail_server_exception);
                break;
            case EZOpenSDKError.ERROR_WEB_DIVICE_VERIFY_CODE_ERROR:
                mVerifyCode = null;
                {
                    // 验证码合法，但是错误
                ...
                break;
            case EZOpenSDKError.ERROR_WEB_DIVICE_SO_TIMEOUT:
                showToast(R.string.device_so_timeout);
                break;
            case EZOpenSDKError.ERROR_WEB_DEVICE_NO_OUT_LIMIT_ERROR:
                // 处理设备添加数目达到上限
                showToast(R.string.device_no_out_limit);
                break;
            case EZOpenSDKError.ERROR_WEB_DEVICE_NOTEXIT:
...
//        mDeviceIcon.setImageResource(getDeviceIcon(""));
    ...
    private void handleQueryCameraFail(final int errCode) {
        mWaitDlg.dismiss();
        switch (errCode) {
            case EZOpenSDKError.VIDEOGONETSDK_PASSWORD_ERROR:
                handleCmaeraPswError();
                break;
            case EZOpenSDKError.VIDEOGONETSDK_DIVICE_VERSION_UNSUPPORT:
            case EZOpenSDKError.VIDEOGONETSDK_DIVICE_UNSUPPORT:
                showErrorPage(R.string.seek_camera_fail_device_not_support_shipin7, 0);
                break;
            case EZOpenSDKError.VIDEOGONETSDK_NETWORD_EXCEPTION:
                showErrorPage(R.string.query_camera_fail_network_exception, 0);
                break;
            case EZOpenSDKError.VIDEOGONETSDK_SERVADDR_EEROR:
            case EZOpenSDKError.VIDEOGONETSDK_SERVER_EXCEPTION:
            ...
            case EZOpenSDKError.ERROR_WEB_HARDWARE_SIGNATURE_ERROR:
                ...
                //ActivityUtils.handleHardwareError(SeriesNumSearchActivity.this, null);
                break;
            case EZOpenSDKError.VIDEOGONETSDK_INPUTPARAM_ERROR:
                showErrorPage(R.string.query_camera_fail_network_exception_or_server_exception, 0);
                break;

            case EZOpenSDKError.ERROR_WEB_DEVICE_ADD_OWN_AGAIN:     // 设备已被自己添加
            case EZOpenSDKError.ERROR_WEB_DIVICE_OFFLINE_BUT_ADD:   //设备不在线，已被别人添加
            ...
            case EZOpenSDKError.ERROR_WEB_DIVICE_ONLINE_ADDED:// 已被其他用户添加
                showUnbind();
                break;

            case EZOpenSDKError.ERROR_WEB_DIVICE_NOT_ONLINE:// 设备不在线, 走wifi配置流程
                showWifiConfig();
                break;

            case EZOpenSDKError.ERROR_WEB_DEVICE_NOTEXIT:// 设备未注册, 或者不在线未添加, 走wifi配置流程
            case EZOpenSDKError.ERROR_WEB_DIVICE_OFFLINE_NOT_ADD:
                showWifiConfig();
                break;
            default:
        ...
        new Thread() {
            public void run() {
                ...
                try {
                    ...
                    if (!TextUtils.isEmpty(mVerifyCode)) {
                        //保存密码
                        mEZOpenSDK.setValidateCode(mVerifyCode, mSerialNoStr);
                    }
