...
public class QueryPlayBackLocalListAsyncTask extends HikAsyncTask<String, Void, Integer> {
    ...
    public QueryPlayBackLocalListAsyncTask(String cameraId, int channelNo, DeviceInfoEx deviceInfoEx,
...
//    	dst.setFileType(src.getFileType());
    ...
    public int searchLocalFile() throws InnerException, HCNetSDKException, CASClientSDKException, PPVClientException {
        ...
        if (deviceInfoEx == null) {
            LogUtil.errorLog(TAG, "searchLocalFile deviceInfoEx is NULL");
            return RemoteListContant.QUERY_EXCEPTION;
        }
        ...
        int playChannelNo = channelNo;
        String mDeviceID = deviceInfoEx.getDeviceID();
        DeviceInfoEx playDeviceInfoEx = deviceInfoEx;
        DeviceInfoEx N1orR1 = null;
        if (deviceInfoEx.getBelongState() == 1 && !Utils.isBlankExt(deviceInfoEx.getBelongSerial())) {
            N1orR1 = DeviceManager.getInstance().getDeviceInfoExById(deviceInfoEx.getBelongSerial());
            if (N1orR1 != null && N1orR1.isOnline()) {
                playChannelNo = deviceInfoEx.getBelongNo();
                playDeviceInfoEx = N1orR1;
                queryMode = DeviceReportInfo.REPOERT_QUERY_RELATE_N1_R1;
                queryDetail = playDeviceInfoEx.getModel() + "," + playDeviceInfoEx.getDeviceID();
            }
        }
//        convertCalendarFiles = remoteListFileCtrl.searchPlayLocalFilesNew(beginDate, endDate, playDeviceInfoEx,
//                playChannelNo, mDeviceID);
		...
		try {
			tmpList = mEZOpenSDK.searchRecordFileFromDevice(mCameraId,
					startTime, endTime);
		} catch (BaseException e) {
			e.printStackTrace();
		}
