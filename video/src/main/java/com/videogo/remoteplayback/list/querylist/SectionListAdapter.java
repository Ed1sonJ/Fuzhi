...
public class SectionListAdapter implements ListAdapter, OnItemClickListener, PinnedHeaderAdapter, SectionIndexer,
    ...
    public SectionListAdapter(final LayoutInflater inflater, final StandardArrayAdapter linkedAdapter,
    ...
    public View getView(final int position, View convertView, final ViewGroup parent) {
        ...
        mImageLoader.cancelDisplayTask(holder.image1);
        mImageLoader.cancelDisplayTask(holder.image2);
        mImageLoader.cancelDisplayTask(holder.image3);
    ...
    private void loadCoverPic(CloudPartInfoFile cloudPartInfoFile, ImageView imageView) {
        String encryptRemoteListPicPasswd = RemoteListUtil.getEncryptRemoteListPicPasswd(deviceInfoEx,
                cloudPartInfoFile.getKeyCheckSum());
        if (needImageLoader(cloudPartInfoFile.getPicUrl(), cloudPartInfoFile.getKeyCheckSum(),
                encryptRemoteListPicPasswd)) {
            String cloudListItemPicUrl = RemoteListUtil.getCloudListItemPicUrl(cloudPartInfoFile.getPicUrl(),
                    cloudPartInfoFile.getKeyCheckSum(), encryptRemoteListPicPasswd);
            imageLoader(cloudListItemPicUrl, imageView);
        } else {
            mImageLoader.cancelDisplayTask(imageView);
            // imageView.setImageResource(R.drawable.playback_cover);
        }
    }

    private void imageLoader(String picUrl, ImageView imageView) {
        DisplayImageOptions options = new DisplayImageOptions.Builder().cacheInMemory(true).cacheOnDisk(true)
                .needDecrypt(false).considerExifParams(true).showImageForEmptyUri(R.drawable.playback_cover2)
                .showImageOnFail(R.drawable.playback_cover2).showImageOnDecryptFail(R.drawable.playback_cover2)
                .method(false).build();

        mImageLoader.displayImage(picUrl, imageView, options, new ImageLoadingListener() {
    ...
    private boolean needImageLoader(String srcPicUrl, String cloudKeyChecksum, String encryptRemoteListPicPasswd) {
        if (TextUtils.isEmpty(srcPicUrl)) {
            return false;
        }
        if (TextUtils.isEmpty(cloudKeyChecksum)) {
            return true;
        }
