...
public class RemoteListUtil {
     ...
     * @since V2.0
     */
    public static ABS_TIME calendarToABS(Calendar timeST) {
        ABS_TIME startTimeST = new ABS_TIME();
        startTimeST.setYear(timeST.get(Calendar.YEAR));
        startTimeST.setMonth(timeST.get(Calendar.MONTH) + 1);
        startTimeST.setDay(timeST.get(Calendar.DAY_OF_MONTH));
        startTimeST.setHour(timeST.get(Calendar.HOUR_OF_DAY));
        startTimeST.setMinute(timeST.get(Calendar.MINUTE));
        startTimeST.setSecond(timeST.get(Calendar.SECOND));
...
/*
        ...
        if (!TextUtils.isEmpty(safePasswd) && safePasswdMd5.equalsIgnoreCase(cloudKeyChecksum)) {
            return safePasswd;
        } else if (!TextUtils.isEmpty(mCloundSafeModePasswd)
                && mCloundSafeModePasswdMd5.equalsIgnoreCase(cloudKeyChecksum)) {
            return mCloundSafeModePasswd;
        }
*/
        return null;
    }
